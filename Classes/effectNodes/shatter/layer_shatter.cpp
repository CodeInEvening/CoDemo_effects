#include "effectNodes/shatter/layer_shatter.h"
...
bool Clayer_shatter::init()
{
    ...
    m_shatterSprite->init("demoRes/1.png");
	...
	for( it = touches->begin(); it != touches->end(); it++)
    {
        ...
        if(m_shatterSprite->getOpacity()==0){
		   ...
		   m_shatterSprite->stopAllActions();
            shatter::CshatterAction*shatter=shatter::CshatterAction::create(8);
            m_shatterSprite->runAction(shatter);
// 			CCMoveTo* t_move = CCMoveTo::create(5, ccp(0,0));
// 			m_shatterSprite->runAction(t_move);
