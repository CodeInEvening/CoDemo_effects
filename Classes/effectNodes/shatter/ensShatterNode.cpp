...
bool CshatterSprite::init(const string&texFileName){
	...
	void CshatterSprite::updateShatterAction(float time,float dt,float growSpeedOfTargetR){
		...
		for(int i=0;i<nRow;i++){
			...
			//	float disToEdgeWithRandom=disToEdge+frag->m_randomNumber%(int)initalTargetR-initalTargetR/2;//add random to disToEdge
				float disToEdgeWithRandom=disToEdge+0-initalTargetR/2;//
				float movLen=disToEdgeWithRandom*0.03333;//we only move some percent of disToEdgeWithRandom
	...
	void CshatterSprite::updateShatterAction_H(float time, float dt, float growSpeedOfTargetR,float s_a,float s_b,float s_c,float s_d)
	{
		CCSize contentSize = this->getContentSize();
		CCPoint center = CCPoint(contentSize.width / 2, contentSize.height / 2);
		float initalTargetR = ccpLength(ccp(contentSize.width, contentSize.height)) / 2;//radius of srounding circle
		...
		for (int i = 0; i < nRow; i++){
			for (int j = 0; j < nCol; j++){
				...
				float dis_movelasttime = s_a*(time - dt)*(time - dt)*(time - dt) + s_b*(time - dt)*(time - dt) + s_c*(time - dt);
				float dis_movenowtime = s_a*time*time*time + s_b*time*time + s_c*time;
				float movLen = dis_movenowtime - dis_movelasttime+(frag->m_randomNumber % (int)initalTargetR)*0.01;
				CCPoint movVec = ccpMult(dir, movLen);
				frag->setPosition(fragPos + movVec);
				...
				float opacity = MAX(0, 255 - 255 * time / growSpeedOfTargetR);
				frag->setOpacity(opacity);
				//update scale
				if (disToCenter >= initalTargetR)
					int tt = 0;
				frag->setScale(MAX(0, m_initalFrageScale - m_initalFrageScale*disToCenter / initalTargetR));
			}
		}
...
void CshatterAction::update(float time){//the time actually is percentage, namely eclipsedTime/totalDuration
	...
	float m_t = getDuration();

  //  ((CshatterSprite*)m_pTarget)->updateShatterAction(m_timeCur,m_timeCur-m_timeFoe,m_growSpeedOfTargetR);
//	float speed_a = (-m_speed_b*m_t*m_t - m_distance) / (2 * m_t*m_t*m_t);
	//float speed_c = (3 * m_distance - m_speed_b*m_t*m_t)/(2*m_t);
	float speed_c = (m_distance-m_speed_b*m_t*m_t)/m_t;
	float speed_a =0;
	if (m_timeCur == 0)
		return;
	((CshatterSprite*)m_pTarget)->updateShatterAction_H(m_timeCur, m_timeCur - m_timeFoe, m_t, speed_a, m_speed_b, speed_c, m_distance);
